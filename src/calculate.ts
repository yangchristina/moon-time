...
const time: [number, number, number, string, string, string][] = [
...
let zodiacYear = function (year: number) {
    const zodiac = ['猴', '鸡', '狗', '猪', '鼠', '牛', '虎', '兔', '龙', '蛇', '马', '羊'];
    return zodiac[year % 12];
}

const lunarYearMeth = function (year: number) {
    const gan = ['庚', '辛', '壬', '癸', '甲', '乙', '丙', '丁', '戊', '己'],
        zhi = ['申', '酉', '戌', '亥', '子', '丑', '寅', '卯', '辰', '巳', '午', '未'],
        str = year.toString().split("");
    return gan[parseInt(str[3])] + zhi[year % 12];
}

const chineseNumber = function (num: number) {
    let res;
...
const getWhetherLeapMonth = (year: number, leapMonth: number) => {
    return time[year][5] == leapMonth.toString() ? true : false;
}

const chineseMonth = function (num: number, leapYear: number) {
    let res;
    const dateHash = ['', '一', '二', '三', '四', '五', '六', '七', '八', '九', '十'];
    if (num <= 10) {
        res = dateHash[num];
...
const getConsecutiveMonths = function (msg: unknown) {
...
export function calculate(obj: GregorianDate & {
    leapMonth?: number
}) {
    ...
    let nowDay = Math.round((new Date(`${obj.year}/${obj.month}/${obj.day}`).getTime() - new Date(`${obj.year}/1/1`).getTime()) / msPerDay) + 1
    ...
    let leapMonth = ruleArr[5] ? ruleArr[5] : null;

    let isLeapMonth = false;

    let getMonth: (() => void) | null = function () {
    ...
    let getIsThirty: (() => void) | null = function () {

        if (thirtyMon.find(e => e == month.toString())) {
        ...
        } else if (ruleArr[4] && twentyNineMon.find(e => e == month.toString())) {
